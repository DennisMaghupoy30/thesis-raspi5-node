...
async function captureFrame(camera) {
    return new Promise(async (resolve, reject) => {
        ...
        if (isWindows) {
            // On Windows, ffmpeg won't work - reject immediately with clear message
            console.error(`Camera ${camera.id}: Cannot capture frame on Windows - ffmpeg requires Linux/USB cameras`);
            reject(new Error('Image capture not supported on Windows'));
            return;
        }
        ...
        ffmpeg.stderr.on('data', (data) => {
            ...
            // Log important errors but filter out normal ffmpeg verbosity
            if (errorText.includes('error') ||
                errorText.includes('failed') ||
                errorText.includes('cannot') ||
                errorText.includes('No such')) {
                console.error(`FFmpeg error for camera ${camera.id}: ${errorText.trim()}`);
            } else if (errorText.includes('frame=') || errorText.includes('fps=')) {
                // Normal ffmpeg output, only log occasionally
                if (Math.random() < 0.1) {
                    console.log(`FFmpeg progress for camera ${camera.id}: ${errorText.trim()}`);
                }
            }
        });
        ...
        ffmpeg.on('close', async (code) => {
            if (code === 0 && imageBuffer.length > 0) {
                ...
                const errorMsg = `FFmpeg failed for camera ${camera.id} (exit code: ${code}, buffer size: ${imageBuffer.length})`;
                if (errorOutput) {
                    console.error(`${errorMsg}\nFFmpeg stderr: ${errorOutput.trim()}`);
                }
        ...
        ffmpeg.on('error', (error) => {
            console.error(`FFmpeg spawn error for camera ${camera.id}: ${error}`);
            reject(error);
        });
    });
}
...
async function predictionLoop() {
    ...
    const predictionPromises = cameras.map(async (camera) => {
        try {
            ...
            console.error(`Camera ${camera.id} capture failed: ${error.message}`);
            // Don't send to prediction API if image capture failed
            return null;
        }
    });
