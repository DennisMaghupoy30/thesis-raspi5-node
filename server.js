...
async function captureFrame(camera) {
    return new Promise(async (resolve, reject) => {
        ...
        if (isWindows) {
            // On Windows, create a proper mock JPEG image for development
            ...
            ]); // Proper minimal JPEG image (1x1 pixel)

            try {
                await writeFile(filepath, mockImageBuffer);
                console.log(`Mock frame captured and saved: ${filepath} (${mockImageBuffer.length} bytes)`);
